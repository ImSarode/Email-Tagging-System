...
tags = {
    ...
    "classes": [r"\bclass\b", r"\bcourse\b", r"\blecture\b", r"\bPOA\b", r"\bPOE\b", r"\bFOE\b", r"\bDSA\b", r"\bIPP\b",
                r"\bIBM\b", r"\bBSAM\b"],
}


class Detagger:
    def _init_(self, filename="detagged_emails.csv"):
    ...
    def load_detagged(self):
        if os.path.exists(self.filename):
            ...
            return set((row["Subject"], row["Received"]) for _, row in df.iterrows())
        return set()
...
def fetch_and_tag_emails():
    ...
    # Put your email in "INPUT_OUTLOOK_EMAIL_HERE"
    ...
    for message in messages:
        ...
        for tag, patterns in tags.items():
            if any(re.search(pattern, subject, re.IGNORECASE) or re.search(pattern, body, re.IGNORECASE) for pattern in
                   patterns):
                tag_list.append(tag)
